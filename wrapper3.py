...
if __name__ == '__main__':
	...
	if not os.path.isdir(pwd + "/bin"):
		...
		sp.Popen("make config.o", shell=True).wait()
		sp.Popen("make semantic.o", shell=True).wait()
		os.chdir("..")
		makefile_ns_p = sp.Popen("make -f Makefile_no_static", shell=True).wait()
		if makefile_ns_p != 0:
			print("Compiling Makefile_no_static failed. Retrying...")
			makefile_p = sp.Popen("make -f Makefile", shell=True).wait()
			if makefile_p != 0:
				os.system("mkdir bin")
				print("Makefile could not be compiled.")
				print("Move binaries into 'bin' directory then retry.\n Exiting.")
				sys.exit(1)
