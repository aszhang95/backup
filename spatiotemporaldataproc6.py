...
class SpatialTemporalData(spatiotemporal_data_):
    ...
    def transform(self, grid_size=None, force=False, date_col="Date", type_col="Primary Type",\
        ...
        '''
        Transform without C++ binary; very slow, needs to be parallelized
        '''
        ...
        self._data_properties_dict["date_range"] = [x.date() for x in date_range.tolist()]
        col_labels += self._data_properties_dict["date_range"]
        count = 0
        ...
        for i in range(grid_size):
            for j in range(grid_size):
                lat_start_ = curr_lat
                lat_stop_ = curr_lat + self._data_properties_dict["delta_lat"]
                lon_start_ = curr_lon
                lon_stop_ = curr_lon + self._data_properties_dict["delta_lon"]
                ...
                (self._data_properties_dict["to_preproc_df"][lon_col] < lon_stop_)].sort_values(date_col)
                ...
                ts = [match.loc[date_range[i]:date_range[i+1]].size for i in np.arange(date_range.size-1)]

                row.extend(ts)
                rows.append(row)
                count += 1
                print("Now processing square {} of {}".format(count, total))
        ...
        grid_df = pd.DataFrame(rows, columns=col_labels)
