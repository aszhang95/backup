...
class SpatialTemporalData(spatiotemporal_data_):
    ...
    def pull(self, dataset_id):
        ...
        client = Socrata(self._livepath, None)
    ...
    def parse_timeseries(self, file_path):
        ...
        date_range = pd.date_range(self._data_properties_dict["min_date"], self._data_properties_dict["max_date"])
        self._data_properties_dict["date_range"] = [x.date() for x in date_range.tolist()]
        return pd.DataFrame(data, columns=(["index"]+self._data_properties_dict["date_range"]), dtype=int).set_index("index")
    ...
    def transform(self, grid_size=200, force=False, date_col="Date", type_col="Primary Type",\
                lat_col="Latitude", lon_col="Longitude", out_fname="grid_df.csv", \
        ...
        '''
        Transform without C++ binary; parallelized
        '''
        ...
        col_labels = ["Event_type", "Latititude_start", "Latitude_stop", "Longitude_start", "Longitude_stop"]
        rows = []
        date_range = pd.date_range(self._data_properties_dict["min_date"], self._data_properties_dict["max_date"], freq=freq)
        self._data_properties_dict["date_range"] = [x.date() for x in date_range.tolist()]
        col_labels.extend(self._data_properties_dict["date_range"])
        print("Length of column labels list = {}".format(len(col_labels)))
        tup_list = []

        for i in range(grid_size):
            for j in range(grid_size):
                tup_list.append((i, j, curr_lat, curr_lon, self._data_properties_dict["delta_lat"], self._data_properties_dict["delta_lon"],\
                self._data_properties_dict["to_preproc_df"], date_col, type_col, type_list, date_range, lat_col, lon_col))
                curr_lon += self._data_properties_dict["delta_lon"]
            curr_lat += self._data_properties_dict["delta_lat"]
            curr_lon = self._data_properties_dict["min_lon"]

        pool = mp.Pool(mp.cpu_count())
        rows = pool.map(todo, tup_list)
        pool.close()
        pool.join()

        print("First row length = {}".format(len(rows[0])))
        self._dataset_df = pd.DataFrame(rows, columns=col_labels)
        self._dataset_df.index.name = "gridsquare_id"
        self._indexmap = self._dataset_df.loc[:, self._dataset_df.columns != 'Event_type']
        ...
        self._data_properties_dict["dataset_df"] = self._dataset_df
        self._dataset_df.to_csv(out_fname)
    ...
    def update(self, fpath, date_col="Date", lat_col="Latitude", lon_col="Longitude",\
        ...
        checklist:
            - update date_range in data_properties_dict
    ...
    def incr_date(self, date):
        '''
        Helper function that extends the date_range of self._dataset_df columns if necessary;
        ...
        if date < self._data_properties_dict["min_date"]: # self._dataset_df.columns[4] needs to be this new date
            date_range = pd.date_range(date, self._data_properties_dict["min_date"])
            self._data_properties_dict["date_range"] = [x.date() for x in date_range.tolist()]
            front = self._dataset_df.columns.tolist()[0:5]
            back = self.dataset_df.columns.tolist()[6:]
            self._dataset_df = self._dataset_df.reindex(\
            columns=(front+self._data_properties_dict["date_range"]+back), fill_value=0)
            # prefix = np.zeros(shape=(self._dataset_df.shape[0], len(date_range)))
            # pre_df = pd.DataFrame(prefix, columns=list(date_range), \
            ...
            date_range = pd.date_range(self._data_properties_dict["max_date"], date)
            self._data_properties_dict["date_range"] = [x.date() for x in date_range.tolist()]
            self._dataset_df = self._dataset_df.reindex(\
            columns=(self._dataset_df.columns.tolist()+self._data_properties_dict["date_range"][1:]), fill_value=0)
            self._data_properties_dict["max_date"] = date
            self.data_properties_dict["dataset_df"] = self._dataset_df
            extended = True
        # else: date w/n date_range
        if extended:
    ...
    def reverse_search(self, date, type_list, date_col="Date", type_col="Primary Type",\
        ...
        res = self._dataset_df[self._dataset_df[search_date] > 0]
        res = res[res["Event_type"] == types]

        # too slow? looping through rows not allowed so what is the best option?
        # parallelize it?
        hit_ids = []
        for dataset_row in res.itertuples(index=True, name="Pandas"):
            ...
            match = self._data_properties_dict["to_preproc_df"].loc[\
            (self._data_properties_dict["to_preproc_df"][lat_col] >= lat_start_) &\
            (self._data_properties_dict["to_preproc_df"][[lat_col] < lat_stop_) &\
            (self._data_properties_dict["to_preproc_df"][[lon_col] >= lon_start_) &\
            (self._data_properties_dict["to_preproc_df"][[lon_col] < lon_stop_) &
            (self._data_properties_dict["to_preproc_df"][date_col] == search_date) &
            (self._data_properties_dict["to_preproc_df"][type_col].isin(types))]

            hit_ids.extend(match[id_col].tolist())
...
def todo(some_tup):
    i, j, curr_lat, curr_lon, dlat, dlon, orig_data, date_col, type_col, type_list, date_range,\
    lat_col, lon_col = some_tup

    lat_start_ = curr_lat
    lat_stop_ = curr_lat + dlat
    lon_start_ = curr_lon
    lon_stop_ = curr_lon + dlon

    # Note: upperbound not inclusive
    match = orig_data.loc[\
    (orig_data[lat_col] >= lat_start_) &\
    (orig_data[lat_col] < lat_stop_) &\
    (orig_data[lon_col] >= lon_start_) &\
    (orig_data[lon_col] < lon_stop_)].sort_values(date_col)
    types_ = set(match[type_col].unique())
    row = [types_, lat_start_, lat_stop_, lon_start_, lon_stop_]

    match.index = match[date_col]
    match = match[[type_col]].isin(type_list)+0.0
    ts = [match.loc[date_range[l]:date_range[l+1]].size for l in np.arange(date_range.size-1)]
    ts.append(match.loc[date_range[date_range.size-1]:date_range[date_range.size-1]].size)
    row.extend(ts)
